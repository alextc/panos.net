namespace PANOS
{
    ...
    public class GetAllAddressesApiResponseResult 
    {
        ...
        private Dictionary<string, AddressObject> AddressObjects
        {
            get
            {
                return AddressesXml.Where(a => a.Netmask != null && !a.Netmask.Contains("/"))
                        .ToDictionary(a => a.Name, a => new AddressObject(a.Name, IPAddress.Parse(a.Netmask), a.Description), StringComparer.OrdinalIgnoreCase);
            }
        }
        ...
        private Dictionary<string, AddressRangeObject> AddressRangeObjects
        {
            get
            {
                return AddressesXml.Where(a => a.Range != null)
                    .ToDictionary(
                        a => a.Name,
                        a => new AddressRangeObject(
                            a.Name,
